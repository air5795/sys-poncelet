...
if (!empty($_POST['companyName']) && !empty($_POST['invoiceId'])) {
    $newInvoiceId = $invoice->duplicateInvoice($_POST['invoiceId']);
    if ($newInvoiceId) {
        echo '<script>window.localStorage.setItem("updateSuccess", "true");</script>';
    } else {
        echo '<script>window.localStorage.setItem("updateSuccess", "false");</script>';
    }
}
			...
			<div class="row">
				<div class="col-md-6">
					...
					<div class="form-group">
						...
						<input data-loading-text="Updating Invoice..." type="submit" name="invoice_btn" value="Duplicar Cotizacion" class="btn btn-info submit_btn invoice-save-btm">
					</div>
